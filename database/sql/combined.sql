-- Combined deployment: relational tables and pgvector tables in one database.
-- Run the whole file in one PostgreSQL database, such as nms_vector.
...
CREATE TABLE aiagent.semantic_memories (
  ...
  memory_id          BIGINT NOT NULL
                     REFERENCES aiagent.long_term_memories(memory_id)
                     ON DELETE CASCADE,
...
CREATE INDEX idx_semantic_memories_model
  ON aiagent.semantic_memories (embedding_model);
...
CREATE TABLE aiagent.knowledge_chunk_embeddings (
  ...
  chunk_id        BIGINT NOT NULL
                  REFERENCES aiagent.knowledge_chunks(chunk_id)
                  ON DELETE CASCADE,
  embedding_model TEXT NOT NULL CHECK (btrim(embedding_model) <> ''),
  embedding       VECTOR(768) NOT NULL,
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE INDEX idx_knowledge_chunk_embeddings_model
  ON aiagent.knowledge_chunk_embeddings (embedding_model);
